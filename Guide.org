...
Modern Emacs includes tree-sitter support, which provides fast and accurate syntax highlighting for JavaScript and TypeScript.  By remapping the default modes to their tree-sitter variants, you get a much better editing experience out-of-the-box.
...
For TypeScript, use the Tide package, which provides deep integration with the TypeScript language server.  Tide handles both syntax checking and intelligent code completion.  You can also chain Tide's TypeScript checker with ESLint:
...
#+BEGIN_SRC elisp
  ...
  (setq ac-auto-start 2) ; start after 2 characters
  (setq ac-ignore-case nil) ; case-sensitive completion
#+END_SRC
