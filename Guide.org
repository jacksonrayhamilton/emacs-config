#+title: Guide to Emacs: How I Use It
...
What follows are all the tips and tricks I have for anyone who wants to become proficient with this awesome tool.  Everything here you can copy into your own config file, which can be saved to: =~/.emacs.d/init.el=

A working config file implementing /everything/ here is also available in this directory; see "init.el"
...
Furthermore, some Emacs modes override these global keybindings with their own defaults.  To ensure your standardized shortcuts work consistently everywhere, you need to explicitly restore them in those modes:
...
It frustrates me that Emacs puts itself out-of-reach to anyone except highly-patient adepts willing to learn its strange default keymap, or else to those possessing the motivation to learn how to fix it.  Thankfully, it only takes a few lines of config to resolve this discomfort and make Emacs "feel like home."
...
When using Ido to open files, there are a couple of customizations that I feel improve the experience:

- =SPACE=, re-bound to itself, allows typing the "Space" character, so as to create new files with spaces in their names.  (Normally =SPACE= does something else in Ido, thus limiting your ability to name files.)
- =Ctrl+.=, typed while selecting a file, instead opens the current directory from the selection interface.  This essentially turns Ido into a high-speed file browser!
...
#+BEGIN_SRC elisp
  ...
  (define-key ido-file-completion-map (kbd "C-.") #'my-ido-select-current-directory)
#+END_SRC

Another thing: Ido has an "auto merge" feature that may search other directories for files when it can't find them in the current directory.  This is enabled by default.  I find it surprising when results are found this way, and I also find that it interferes with Ido's capability to create new files by typing names that don't yet exist in the current directory.  For those reasons, I prefer to disable auto merging:
...
Note: by default, Emacs downcases text pasted into the search buffer.  Since this can interfere with obtaining exact matches for code, I prefer to disable that behavior:
...
Once, you will need to install the grammars for each of these modes.  You will need a C/C++ compiler and linker installed in order to do this.  You can install grammars from within Emacs, but I prefer to run a script to do it:
...
I love having "squiggly lines" in my editor; I want to know about potential problems in my code /before/ I go to test it.  FlyCheck will utilize my ESLint installation to check my JavaScript for issues, and it will report any compilation errors from my TypeScript language server when I'm working with TypeScript.
...
Something else to note: by default, =auto-complete= provides suggestions that are case-insensitive matches to what you typed, but it will dynamically begin to narrow its suggestions to case-sensitive matches if you type a capital letter.  I imagine this behavior was intended to enable users to auto-complete without using the "Shift" key, but in my experience the suggestions just tend to feel less predictable.  You're also less likely to receive a single suggestion, which would be speediest.  Therefore, I prefer to make =auto-complete= unconditionally case-sensitive:
...
Emacs has oodles of miscellaneous settings that you can (and probably will want to) customize.  There are too many for me to list here and their usefulness will depend on your usage patterns.

However, there are a few features that I can't imagine not having turned-on.

*** Persistence Across Sessions

If I ever want to restart Emacs (such as to try out config changes, or due to restarting my computer), then it'd be nice if Emacs would reopen all the files I had open, avoiding any disruption to my ongoing tasks.  =desktop-save-mode= does exactly that.

If you ever want to drop all your open files, you can just run =desktop-clear= via =Alt+X=.
...
If you write software with others, then it's good for team cohesion and code quality to have a consistent set of formatting rules everyone follows.  EditorConfig largely solves that problem.  As long as you have =.editorconfig= files in all the projects you work on, you won't need to configure Emacs to format code in a particular way, nor will you need to change your configuration whenever a project's conventions don't agree with your defaults.

#+BEGIN_SRC elisp
  (editorconfig-mode 1)
#+END_SRC

*** Automatic Parenthesis, Bracket, Brace, & Quote Pairing

Always having these syntax elements balanced is convenient when you're writing code.  It helps to avoid syntax errors.  It's even useful for quotes when you're writing in a natural language.
...
It's possible to install and manage your packages entirely inside Emacs.  That said, I prefer to install my packages via a script; that way, Emacs is "ready-to-go" the first time I open it, without me ever needing to use a "package-less" version to set things up.  To install every package I mentioned throughout this guide, run the following command:
...
And make sure to run these scripts to install all the necessary dependencies:
