...
Something else to note: by default, =auto-complete= uses dynamic case matching, which I feel can lead to unexpected completion suggestions.  When I set it to be unconditionally case-sensitive, I find that the suggestions are slightly more predictable and thus more useful for coding:
