...
- =Ctrl+O=, typed a second time while selecting a file, instead opens the current directory from the selection interface.  This essentially turns Ido into a high-speed file browser!
- =SPACE=, re-bound to itself, allows typing the "Space" character, so as to create new files with spaces in their names.  Normally this is not possible in Ido, so having this ability enables Ido to either find OR create any file.
